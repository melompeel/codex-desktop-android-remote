...
- 侧栏默认进入“最近任务”，“所有任务”位于第二项；底部仅保留紧凑的“通过VPN/代理”开关。
- 运行中的任务会在卡片右侧显示进度动画。
- 任务列表中的“桌面已打开”表示当前 Desktop owner 可接受远程操作。
- 点击“历史”任务时，App 会请求 Windows 自动在 Codex Desktop 中载入该对话；成功后恢复发送、停止和推送能力，失败时仍可查看历史。
...
- 长任务首次只请求最近 50 条时间线记录，同时保留最近一次用户要求；向上滚动到顶部后自动加载两页更早记录，再显示“加载全部历史记录”，失败时可直接重试。
- 任务列表和最近查看的文字会话按终端缓存在本地，重新打开时会先展示缓存并在后台同步最新记录；缓存总量限制为 50 MB，每台终端最多保留最近 32 个会话。
...
- Wi-Fi、移动数据或 VPN 路径发生变化时，App 会主动重建连接，不再等待最长退避超时；局域网直连暂时断网后也会在网络恢复时再次连接。
